...
[assembly: InternalsVisibleTo("bwdk_sdk.Test")]
...
namespace bwdk_sdk.Client
{
    ...
    public static partial class ClientUtils
    {
        ...
        public static string? ParameterToString(object? obj, string? format = ISO8601_DATETIME_FORMAT)
        {
            ...
            if (obj is bool boolean)
                return boolean
                    ? "true"
                    : "false";
            if (obj is ErrorEnum errorEnum)
                return ErrorEnumValueConverter.ToJsonValue(errorEnum);
            if (obj is GatewayTypeEnum gatewayTypeEnum)
                return GatewayTypeEnumValueConverter.ToJsonValue(gatewayTypeEnum).ToString();
            if (obj is NullEnum nullEnum)
                return NullEnumValueConverter.ToJsonValue(nullEnum);
            if (obj is OrderStatusEnum orderStatusEnum)
                return OrderStatusEnumValueConverter.ToJsonValue(orderStatusEnum).ToString();
            if (obj is ShippingTypeEnum shippingTypeEnum)
                return ShippingTypeEnumValueConverter.ToJsonValue(shippingTypeEnum).ToString();
            if (obj is TypeNameEnum typeNameEnum)
                return TypeNameEnumValueConverter.ToJsonValue(typeNameEnum);
        ...
        /// </summary>
        public const string BASE_ADDRESS = "https://bwdk-backend.digify.shop";
        ...
        /// The host of the API
        /// </summary>
        public const string HOST = "bwdk-backend.digify.shop";
